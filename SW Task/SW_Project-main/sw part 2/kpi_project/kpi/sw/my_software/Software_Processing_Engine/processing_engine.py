...
from ..Software_Processing_Engine.lexer import Lexer
from ..Software_Processing_Engine.my_parser import Parser
from ..Software_Processing_Engine.interpreter import Interpreter
from ..Software_Output_Message_Producer.output_message_producer import OutputMessageProducer
class ProcessingEngine():
    ...
    def __init__(self,processing_engine_arg=None,data_sink_args=None):
        ...
        """
        self.data_source = DataSourceFactory.create_data_source(*processing_engine_arg)
    ...
    def start(self,current_msg):
        ...
        value=current_msg.get("value")

        if isinstance(value, str):
            string_value = value  # No need to join or convert if it's already a string
        else:
            string_value = ''.join(value.astype(str).tolist())       
        parser.attr_value=string_value
